...
class CharacterVideoMaker:
    ...
    def create_frame_with_text(self, title, voice_text, x, y):
        ...
        formation.alpha_composite(Image.composite(Empty, Tachie, maskImg),
                                  ((-round(TachieWidth / 2) + 520) + x, 90 - y))
        ...
        final_draw.text((834, 45), Char_CN_Name, font=font_name, fill=text_color)
        ...
        height = 120 + accentCN - ascentEN
        '''
        print(height, ascentEN, descentEN, accentCN)
        '''
        final_draw.text((834, height), Char_EN_Name, font=font_ENname, fill=text_color)

        # 标题文本（水平和垂直居中，补偿Bold字体偏移）
        final_draw.text((912, 321), title, font=font_title, fill=text_color)

        cv_text = f"CV: {self.cv_name}"
        final_draw.text((850,900), cv_text, font=font_cv, fill=text_color)
    ...
    def process_single_audio(self, audio_file, voice_data, x, y):
        ...
        video = video.with_effects([vfx.CrossFadeIn(1)]).copy()
        video = video.with_effects([vfx.CrossFadeOut(1)]).copy()
    ...
    def Tachie_Check(self):
        ...
        Tachie = Image.open(char_image_path)
        Tachie = Tachie.convert("RGBA")
        TachieRuler = Image.open("./resources/TachieRuler.png")
        ...
        while True:
            ...
            final_draw.text((834, 45), Char_CN_Name, font=font_name, fill=text_color)
            width = final_draw.textlength(text=Char_CN_Name) + 835
            print(type(width))
            accentCN, descentCN = font_name.getmetrics()
            ascentEN, descentEN = font_ENname.getmetrics()
            height = 120 + accentCN - ascentEN
